...
func TestPoliCheckinBooking(t *testing.T) {
	assert := assert.New(t)
	...
	t.Run("Poli Checkin Success", func(t *testing.T) {
		...
		err := bookingService.CheckInPoliBooking(command)
		assert.Nil(err)
	})

	t.Run("Poli Checkin Error because Event Publisher Error", func(t *testing.T) {
		bookingRepoMock := new(repo_mocks.BookingRepoMock)
		bookingRepoMock.On("Get", booking.ID).Return(booking, nil)
		bookingService := service.ServiceImpl{
	...
	t.Run("Poli Checkin Error because Booking Not Found", func(t *testing.T) {
		bookingRepoMock := new(repo_mocks.BookingRepoMock)
